...
public class BookingServiceImpl implements BookingService {
    ...
    public Mono<Ticket> bookFlight(String flightId, BookingRequest bookingRequest) {
        ...
        Mono<User> userMono = userRepository.findByEmail(bookingRequest.getEmail())
        ...
        return Mono.zip(flightMono, userMono)
                .flatMap(tuple -> {
                    ...
                    if (flight.getAvailableSeats() < requestedSeats) {
                        return Mono.error(new UnprocessableException("Not enough seats available"));
                    }
                    ...
                    if (bookingRequest.getPassenger() != null) {
                        ticket.setPassenger(
                            bookingRequest.getPassenger().stream()
        ...
        return ticketRepository.findByPnr(pnr)
                ...
                .flatMap(ticket ->
                    flightRepository.findById(ticket.getFlightId())
                            .switchIfEmpty(Mono.error(new ResourceNotFoundException("Flight not found for ticket")))
                            .flatMap(flight -> {
